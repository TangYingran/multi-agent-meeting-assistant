...
from ..integrations.idempotency_store import IdempotencyStore
...
class ActionAgent:
    ...
    async def _sync_to_external(
        ...
        synced = []
        for item in items:
            idem_key = self.idempotency_store.build_action_key(
                meeting_id=meeting_id,
            ...
            if existing and (
                existing.get("jira_issue_key") or existing.get("feishu_task_id")
            ):
                item.jira_issue_key = existing.get("jira_issue_key")
                item.feishu_task_id = existing.get("feishu_task_id")
                logger.info(
                    f"[ActionAgent] Idempotent hit, reuse external ids: {item.task}"
                )
                synced.append(item)
                continue
            ...
            self.idempotency_store.save_result(
                ...
                feishu_task_id=item.feishu_task_id,
                status="synced"
                if (item.jira_issue_key or item.feishu_task_id)
                else "pending",
            )
            synced.append(item)
