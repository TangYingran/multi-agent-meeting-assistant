...
def _prefer_non_empty(existing: Any, new: Any) -> Any:
    """
    合并并行分支的通用 reducer。

    规则：
    - 旧值为空(None/""/b"")时采用新值
    - 旧值非空时保留旧值，避免并行分支互相覆盖
    """
...
class GraphState(TypedDict, total=False):
    ...
    """
    meeting_id: Annotated[str, _prefer_non_empty]
    status: Annotated[str, _prefer_non_empty]
    audio_data: Annotated[bytes, _prefer_non_empty]

    # Transcription 输出
    transcript: Annotated[Any, _prefer_non_empty]
    transcript_text: Annotated[str, _prefer_non_empty]
